...
function beginStudyGame(actionId) {
const academy = gameState.academy;
const action = academyActions[actionId];
const game = studyGames[actionId];
if (!action || !game || academy.actionPoints <= 0) return;
if (academy.dailyActions.includes(actionId)) return;
if (academy.stress >= academy.maxStress && actionId !== "rest") return;

const questionIndex = academy.actionCounts[actionId] % game.questions.length;
const question = game.questions[questionIndex];
academy.studyGame = {
actionId,
questionIndex,
order: shuffledQuestionChoices(question),
selected: null,
phase: "question",
correct: null
};
gameState.currentScene = "studyGame";
renderStudyGame();
}

function createQuizChoice(text, displayIndex, selected) {
const button = document.createElement("button");
...
button.addEventListener("click", () => {
gameState.academy.studyGame.selected = displayIndex;
renderStudyGame();
});
return button;
}
...
const state = gameState.academy.studyGame;
if (!state || !studyGames[state.actionId]) {
renderAcademy();
return;
}
const game = studyGames[state.actionId];
const question = game.questions[state.questionIndex];
gameState.currentScene = "studyGame";
elements.chapterTitle.textContent = game.name;
elements.sceneLocation.textContent = `修習第${gameState.academy.day}日`;
elements.speakerName.textContent = "掌院試題";
elements.dialogueText.textContent = question.prompt;
elements.progressText.textContent = "作答中";
hideFeedback();
updateStats();
elements.choicesContainer.replaceChildren();

if (state.phase === "result") {
renderStudyGameResult(state, game, question);
saveGame(true);
return;
}

state.order.forEach((originalIndex, displayIndex) => {
elements.choicesContainer.appendChild(
createQuizChoice(question.choices[originalIndex], displayIndex, state.selected === displayIndex)
);
});

const submit = document.createElement("button");
submit.className = "choice-button";
submit.textContent = "提交答案";
submit.disabled = state.selected === null;
submit.addEventListener("click", submitStudyGame);
elements.choicesContainer.appendChild(submit);
saveGame(true);
}

function submitStudyGame() {
const academy = gameState.academy;
const state = academy.studyGame;
if (!state || state.phase !== "question" || state.selected === null) return;
const game = studyGames[state.actionId];
const question = game.questions[state.questionIndex];
const originalIndex = state.order[state.selected];
state.correct = originalIndex === question.correct;
state.phase = "result";

const action = academyActions[state.actionId];
if (state.correct) {
...
academy.dailyActions.push(state.actionId);
academy.actionCounts[state.actionId] = (academy.actionCounts[state.actionId] || 0) + 1;
renderStudyGame();
}

function renderStudyGameResult(state, game, question) {
const selectedOriginal = state.order[state.selected];
const selectedText = question.choices[selectedOriginal];
const correctText = question.choices[question.correct];
elements.progressText.textContent = state.correct ? "作答正確" : "完成訂正";
showFeedback({
title: state.correct ? "答對" : "答案不正確",
text:
`【你的答案】${selectedText}\n\n` +
`【正確答案】${correctText}\n\n` +
...
continueButton.textContent = "完成修習";
continueButton.addEventListener("click", finishStudyGame);
...
title: `${game.name}｜${wasCorrect ? "完成" : "已訂正"}`,
text: wasCorrect
? "你已根據篇章內容完成本次修習。能力獎勵已結算。"
: "你已查看答案與解析。本次不獲能力獎勵，但修習紀錄與心疲仍會保留。"
};
academy.lastFeedback = actionFeedback;
academy.studyGame = null;
...
function loadGame() {
  ...
  try {
    ...
    if (gameState.currentScene === "studyGame" && gameState.academy.studyGame) {
renderStudyGame();
} else if (gameState.currentScene === "badEnding" && gameState.failure) {
